...
# runs its action (which is to call "make cronac")
# ---------------------------------------------
all: 
	make cronac
...
# compiler. Note here the difference with the cronac rule:
...
# crona.l (hence crona.l is a dependency of lexer.yy.cc). 
...
# Run the program on test1.crona, writing the
...
# ---------------------------------------------
test: all
	./cronac test1.crona -t test1.out 2> test1.err
